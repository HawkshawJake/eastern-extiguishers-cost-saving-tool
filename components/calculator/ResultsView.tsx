...
import { TrendingDown, Leaf, Calendar, Info } from 'lucide-react'
...
import { COMPARISON_YEARS } from '@/data/extinguishers'
import { addEntry, updateEntryContact } from '@/lib/eventStore'
import { getStored, setStored } from '@/lib/safeStorage'
import LeadCaptureModal, { type Viewport } from '@/components/LeadCaptureModal'
...
interface Props {
  /** 'website' files the lead against the standing Website session. */
  source?: 'website'
  onRecalculate: () => void
  onStartOver: () => void
  /** Passed through to the lead modal when running inside an iframe. */
...
export default function ResultsView({ source, onRecalculate, onStartOver, viewport }: Props) {
  ...
  // The figure stored against the lead always uses the standard comparison
  // period, whatever the visitor drags the slider to afterwards.
  const defaultTotals = useMemo(
    () => calcTotals(steelInventory, p50Inventory, steelTypes, p50Types, COMPARISON_YEARS, constants),
    [steelInventory, p50Inventory, steelTypes, p50Types, constants],
  )

  useEffect(() => {
    const storedId = getStored('ee_entry_id')
    ...
    addEntry({
      ...
      session_id: getStored('ee_session_id') ?? undefined,
      source,
    }).then(id => {
      if (id) {
        entryIdRef.current = id
        setStored('ee_entry_id', id)
      }
    }).catch(() => { /* non-critical */ })
  ...
  useEffect(() => {
    if (defaultTotals.saving <= 0) return
    if (getStored('ee_lead_done')) return
  ...
  async function handleLeadSubmit(data: {
    ...
    if (entryIdRef.current) {
      ...
      // addEntry hadn't resolved yet — write a complete record instead.
  ...
  const breakEvenYear = useMemo(
    () => findBreakEvenYear(
      calcCumulativeCosts(steelInventory, p50Inventory, steelTypes, p50Types, 30, constants),
    ),
    [steelInventory, p50Inventory, steelTypes, p50Types, constants],
  )
  ...
  const breakEvenLabel =
    breakEvenYear === null ? '' : breakEvenYear < 1 ? 'Immediate' : `Year ${Math.ceil(breakEvenYear)}`
  ...
  return (
    <>
      ...
      {breakEvenYear !== null ? (
        ...
        >
          <Calendar className={breakEvenWithinRange ? 'text-eco-green' : 'text-blue-400'} size={20} />
          <div>
            <p
            ...
            <p className="font-body font-semibold text-brand-black">{breakEvenLabel}</p>
            {!breakEvenWithinRange && (
              <p className="font-body text-xs text-gray-500 mt-0.5">
                Extend the comparison period to see this on the chart
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-md p-4 mb-5 flex items-start gap-3">
          ...
          <p className="font-body text-sm text-amber-700">
            P50 does not pay back within 20 years for this inventory mix. Check your quantities are
            set correctly.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button className="btn-secondary flex-1" onClick={onRecalculate}>
          ← Recalculate
        </button>
        <button className="btn-primary flex-1" onClick={() => setShowModal(true)}>
          Get My Report
        </button>
        <button className="btn-secondary flex-1" onClick={onStartOver}>
          Start Over
        </button>
      </div>

      <p className="text-center text-xs text-gray-400 font-body mt-6">
        Figures are estimates based on typical service, exchange, and disposal charges. Speak to an
        Eastern Extinguishers advisor for a tailored quote.
      </p>
    </>
  )
}
